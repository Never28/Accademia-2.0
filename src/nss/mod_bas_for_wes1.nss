void main()
{
    ...
    while(GetIsItemPropertyValid(ip_Item)){
        ...
        if(nType == ITEM_PROPERTY_WEIGHT_INCREASE){
    ...
    (GetItemPossessedBy(oPC,"crystal_pos_1")!=OBJECT_INVALID)){
        ...
        if(nGold>=32000){
            ...
            AddItemProperty(DURATION_TYPE_PERMANENT,ItemPropertyWeightIncrease(IP_CONST_WEIGHTINCREASE_5_LBS),oWeap);
            AddItemProperty(DURATION_TYPE_PERMANENT,ItemPropertyDamageBonus(IP_CONST_DAMAGEBONUS_1,DAMAGE_TYPE_POSITIVE),oWeap);
            TakeGoldFromCreature(2000,oPC,TRUE);
            SetStolenFlag(oWeap,TRUE);
