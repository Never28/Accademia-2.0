...
void main()
{
    ...
    object oSlotPC = GetItemInSlot(INVENTORY_SLOT_CHEST,oAttacker);
    string sRefVeste = GetResRef(oSlotPC);
    if(sRefVeste == "loot_spia" && GetIsPC(oAttacker) == TRUE){
        DestroyObject(oSlotPC,0.0);
        SetLocalInt(oAttacker,"spiacammuffamento",0);
    }
