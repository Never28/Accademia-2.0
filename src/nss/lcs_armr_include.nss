//:: Original by Lilista
//:: Modified by Cuprius on 5/7/2008 to reflect CEP 2.1 content
//:: - Set part number limits to 0-210 for torsos and 0-199 for other parts.
//:: - Removed intermediate conditional statements in search loops - increases
//::   number of 2da lookups but decreases number of computing steps
...
int lcs_GetNextValidArmor2DALine(object oItem, int nPart)
{
    ...
    if(nPart != ITEM_APPR_ARMOR_MODEL_TORSO)
    {
        while(sCurrent2DA == "")
        {
            ...
            // Removed intermediate value conditionals - Cuprius
            else
            {
                nAppearance = nAppearance + 1;
            }
...
void lcs_ModifyandEquipNewArmor(object oItem, int nPart, int nAppearance)
{
    ...
    if(!GetIsObjectValid(oNewItem)) return;
    DestroyObject(oItem);
