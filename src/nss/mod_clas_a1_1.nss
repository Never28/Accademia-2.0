void main()
{
    ...
    if(GetIsFriend(oTarget,oPC)==FALSE){
        if(nAlign == ALIGNMENT_GOOD){
            ...
            if(nAttack >= nAC){
                int nDC = 50;
                effect eDamage = EffectDamage(nLevel,DAMAGE_TYPE_NEGATIVE);
                effect eVis = EffectVisualEffect(VFX_IMP_HARM);
                eDamage = EffectLinkEffects(eVis,eDamage);
                if(WillSave(oTarget,nDC,SAVING_THROW_TYPE_NEGATIVE,oPC)==0){
            ...
            SendMessageToPC(oPC,"Bersaglio di Allineamento non Buono");
    }
    else
