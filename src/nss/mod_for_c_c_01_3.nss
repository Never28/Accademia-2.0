void CreaOggetto(object oPC){
    CreateItemOnObject("set_01_w_ans",oPC);
}
...
void main()
{
    ...
    int nManRat=4,nSpiOmb=2,nEssAll=2,nDiff_1,nDiff_2,nDiff_3;
    if(nGold>=2000){
        while(oItem!=OBJECT_INVALID){
            sTag = GetTag(oItem);
            if(sTag=="craft_1_rat_bos2"&&nManRat>0){
                nDiff_1 = GetItemStackSize(oItem);
                if(nDiff_1>nManRat){
                    SetItemStackSize(oItem,nDiff_1-nManRat);
                    nManRat=0;
                }
                else if(nDiff_1==nManRat){
                    DestroyObject(oItem,0.0);
                    nManRat=0;
                }
                else{
                    DestroyObject(oItem,0.0);
                    nManRat=nManRat-nDiff_1;
                }
            }
            else if(sTag=="craft_1_all"&&nEssAll>0){
                nDiff_2 = GetItemStackSize(oItem);
                if(nDiff_2>nEssAll){
                    SetItemStackSize(oItem,nDiff_2-nEssAll);
                    nEssAll=0;
                }
                else if(nDiff_2==nEssAll){
                    DestroyObject(oItem,0.0);
                    nEssAll=0;
                }
                else{
                    DestroyObject(oItem,0.0);
                    nEssAll=nEssAll-nDiff_2;
                }
            }
            else if(sTag=="craft_1_omb"&&nEssAll>0){
                nDiff_3 = GetItemStackSize(oItem);
                if(nDiff_3>nSpiOmb){
                    SetItemStackSize(oItem,nDiff_3-nSpiOmb);
                    nSpiOmb=0;
                }
                else if(nDiff_3==nSpiOmb){
                    DestroyObject(oItem,0.0);
                    nSpiOmb=0;
                }
                else{
                    DestroyObject(oItem,0.0);
                    nSpiOmb=nSpiOmb-nDiff_3;
                }
            }
