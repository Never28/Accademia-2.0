void CreaOggetto(object oPC){
    CreateItemOnObject("set_05_w_and",oPC);
}
...
void main()
{
    ...
    int nItem1=6,nItem2=6,nItem3=4,nDiff_1,nDiff_2,nDiff_3;
    if(nGold>=2000){
        while(oItem!=OBJECT_INVALID){
            sTag = GetTag(oItem);
            if(sTag=="craft_5_orm"&&nItem2>0){
                nDiff_1 = GetItemStackSize(oItem);
                if(nDiff_1>nItem2){
                    SetItemStackSize(oItem,nDiff_1-nItem2);
                    nItem2=0;
                }
                else if(nDiff_1==nItem2){
                    DestroyObject(oItem,0.0);
                    nItem2=0;
                }
                else{
                    DestroyObject(oItem,0.0);
                    nItem2=nItem2-nDiff_1;
                }
            }
            else if(sTag=="craft_5_orf"&&nItem3>0){
                nDiff_2 = GetItemStackSize(oItem);
                if(nDiff_2>nItem3){
                    SetItemStackSize(oItem,nDiff_2-nItem3);
                    nItem3=0;
                }
                else if(nDiff_2==nItem3){
                    DestroyObject(oItem,0.0);
                    nItem3=0;
                }
                else{
                    DestroyObject(oItem,0.0);
                    nItem3=nItem3-nDiff_2;
                }
            }
            else if(sTag=="craft_5_gos"&&nItem1>0){
                nDiff_3 = GetItemStackSize(oItem);
                if(nDiff_3>nItem1){
                    SetItemStackSize(oItem,nDiff_3-nItem1);
                    nItem1=0;
                }
                else if(nDiff_3==nItem1){
                    DestroyObject(oItem,0.0);
                    nItem1=0;
                }
                else{
                    DestroyObject(oItem,0.0);
                    nItem1=nItem1-nDiff_3;
                }
            }
