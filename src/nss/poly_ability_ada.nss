...
void AuraDiMutaE(object oPC,int nTime,int nLevelB, int nDam,int nDmgRedB){
    int AuraDiMutaE_CURRENT = GetLocalInt(oPC,"AuraDiMutaE_CURRENT");
    if(AuraDiMutaE_CURRENT != nTime && AuraDiMutaE_CURRENT != 0){
        AuraDiMutaE_CURRENT = AuraDiMutaE_CURRENT +1;
        effect eAOE = EffectVisualEffect(VFX_DUR_AURA_BLUE);
        effect eAttack = EffectAttackIncrease(nLevelB,ATTACK_BONUS_MISC);
        effect eDamage = EffectDamageIncrease(nDam, DAMAGE_TYPE_ELECTRICAL);
        ...
        SetLocalInt(oPC,"AuraDiMutaE_CURRENT",AuraDiMutaE_CURRENT);
        DelayCommand(6.0,AuraDiMutaE(oPC,nTime,nLevelB,nDam,nDmgRedB));
    }
    else
...
void main()
{
    ...
    int nAttackB,nDamageB,nSaveB,nDmgRedB,nDmgRedP,nLevel,nLevelB,AbilityBlack_CURRENT;
            AbilityBlack_CURRENT = GetLocalInt(oPC,"AuraDiMutaE_CURRENT");
            if(AbilityBlack_CURRENT == 0){
                ...
                nLevelB = (nLevel / 3);
                eVis = EffectVisualEffect(VFX_IMP_HEAD_ELECTRICITY);
                eVis = EffectLinkEffects(EffectVisualEffect(VFX_FNF_LOS_NORMAL_30),eVis);
                ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oPC);
                SetLocalInt(oPC,"AuraDiMutaE_CURRENT",1);
                ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eAOE, OBJECT_SELF, 6.0);
                nDmgRedB = 12;
                AuraDiMutaE(oPC,nTime,nLevelB,nDam,nDmgRedB);
            }
}
