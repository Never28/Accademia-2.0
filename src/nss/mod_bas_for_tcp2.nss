...
void main()
{
    ...
    int nControll = 0;
    if(sResRef=="weap_bas_axt_4")
        nControll = 1;
    else if(sResRef=="weap_bas_dar_4")
        nControll = 2;
    else if(sResRef=="weap_bas_shu_4")
        nControll = 3;
    if(nControll != 0 && (GetItemPossessedBy(oPC,"crystal_pos_2")!=OBJECT_INVALID)){
        ...
        if(nGold>=32000){
            while(oItem!=OBJECT_INVALID){
                ...
                if(sTag=="crystal_pos_2"&&nItem1>0){
                ...
                else if(sResRef2=="weap_bas_axt_4" && nControll == 1 && oItem != oWeap){
                    int nControll2 = 0;
                ...
                else if(sResRef2=="weap_bas_dar_4" && nControll == 2 && oItem != oWeap){
                    int nControll2 = 0;
                    itemproperty ip_Item2 = GetFirstItemProperty(oItem);
                    while(GetIsItemPropertyValid(ip_Item2)){
                        int nType2 = GetItemPropertyType(ip_Item2);
                ...
                else if(sResRef2=="weap_bas_shu_4" && nControll == 3 && oItem != oWeap){
                    int nControll2 = 0;
                    itemproperty ip_Item2 = GetFirstItemProperty(oItem);
                    while(GetIsItemPropertyValid(ip_Item2)){
                        int nType2 = GetItemPropertyType(ip_Item2);
            ...
            IPSafeAddItemProperty(oWeap, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_POSITIVE,IP_CONST_DAMAGEBONUS_1d6));
