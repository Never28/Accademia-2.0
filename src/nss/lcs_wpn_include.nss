...
int lcs_GetWeaponModelLowerBound(object oItem, int nPart)
{
    ...
    if(nPart == ITEM_APPR_WEAPON_MODEL_TOP)
    {
        switch(nWeaponType)
        {
            ...
            case BASE_ITEM_LIGHTFLAIL       : return 5;
            ...
            case BASE_ITEM_TRIDENT          : return 4;
        }
    }
...
int lcs_GetNextValidWeaponModel(object oItem, int nPart)
{
    ...
    if((GetBaseItemType(oItem) == BASE_ITEM_MAGICSTAFF) && (nCurrentAppearance == 6)){
        int nQuarter = GetItemAppearance(oItem, ITEM_APPR_TYPE_WEAPON_COLOR, nPart);
        if(nQuarter > 1)
            lcs_ModifyColorandEquipNewWeapon(oItem, nPart, 1);
        return 7;
    }
    if((GetBaseItemType(oItem) == BASE_ITEM_GREATSWORD) && (nCurrentAppearance == 4))
    {
        return 7;
    }
    if((GetBaseItemType(oItem) == BASE_ITEM_GREATSWORD) && (nCurrentAppearance == 7))
    {
        return 1;
    }
...
int lcs_GetPreviousValidWeaponModel(object oItem, int nPart)
{
    ...
    int nCurrentAppearance = GetItemAppearance(oItem, ITEM_APPR_TYPE_WEAPON_MODEL, nPart);
    if((GetBaseItemType(oItem) == BASE_ITEM_MAGICSTAFF) && (nCurrentAppearance == 1)){
        int nQuarter = GetItemAppearance(oItem, ITEM_APPR_TYPE_WEAPON_COLOR, nPart);
        if(nQuarter > 1)
            lcs_ModifyColorandEquipNewWeapon(oItem, nPart, 1);
        return 7;
    ...
    if((GetBaseItemType(oItem) == BASE_ITEM_GREATSWORD) && (nCurrentAppearance == 1))
    {
        return 7;
    }
...
int lcs_GetNextValidWeaponColor(object oItem, int nPart)
{
    ...
    if(GetBaseItemType(oItem) == BASE_ITEM_MAGICSTAFF && GetItemAppearance(oItem, ITEM_APPR_TYPE_WEAPON_MODEL, nPart) == 7){
        return 1;
    }
    if(nAppearance == 4)
    {
