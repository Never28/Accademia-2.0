...
void main()
{
    ...
    effect eMissile = EffectBeam(VFX_BEAM_BLACK,oArmaSpec,BODY_NODE_CHEST);
    ...
    if(nAttack>=nAC){
        if(!MySavingThrow(SAVING_THROW_FORT, oTarget, nDC,SAVING_THROW_TYPE_POISON,oPC)){
            effect eVenom = EffectAbilityDecrease(ABILITY_CONSTITUTION,8);
