    ...
    if(GetHitDice(OBJECT_SELF)==40)
        nSpellDC = nSpellDC +4;
            if(GetHasFeat(FEAT_EPIC_SPELL_FOCUS_EVOCATION,OBJECT_SELF))
                nSpellDC = nSpellDC +6;
            else if(GetHasFeat(FEAT_SPELL_FOCUS_EVOCATION,OBJECT_SELF))
                nSpellDC = nSpellDC + 2;
            else if(GetHasFeat(FEAT_GREATER_SPELL_FOCUS_EVOCATION,OBJECT_SELF))
                nSpellDC = nSpellDC + 4;
    ...
    while (GetIsObjectValid(oTarget))
    {
        ...
        fDelay = GetDistanceBetweenLocations(lTarget, GetLocation(oTarget))/20 + 0.5f;
           //Roll damage for each target
            nDamage1 = d12(nSpec*2);
            float fDamage = (IntToFloat(nDamage1)/100.0)*75.0;
            nDamage1 = FloatToInt(fDamage);
            nDamage2 = nDamage1/5;
            nDamage3 = nDamage1/5;
            nDamage4 = nDamage1/5;
            nDamage5 = nDamage1/5;
            ...
            eDam5 = EffectDamage(nDamage5, DAMAGE_TYPE_COLD);
            if(nTotalDamage > 0)
            {
                if (nTotalDamage > 50)
                {
