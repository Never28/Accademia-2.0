...
void main()
{
    string sResf = GetResRef(OBJECT_SELF);
    if(sResf=="set_46_bos" &&(GetObjectByTag("sek_1")!=OBJECT_INVALID||GetObjectByTag("sek_2")!=OBJECT_INVALID||
    GetObjectByTag("sek_3")!=OBJECT_INVALID||GetObjectByTag("sek_4")!=OBJECT_INVALID)){
        effect eRaise = EffectResurrection();
        effect eHeal = EffectHeal(412);
        effect eVis = EffectVisualEffect(VFX_IMP_RAISE_DEAD);
        DelayCommand(0.8,ApplyEffectToObject(DURATION_TYPE_INSTANT, eRaise, OBJECT_SELF));
        DelayCommand(1.2,ApplyEffectToObject(DURATION_TYPE_INSTANT, eHeal, OBJECT_SELF));
        DelayCommand(2.0,AssignCommand(OBJECT_SELF,ActionSpeakString("Non puoi uccidere un eletto di Sekolah viscido uomo terrestre!")));
    }
    else{
            if(sResf=="set_47_bos"){
                SetLocalInt(GetModule(),"n_spell_s1",d10(1));
                SetLocalInt(GetModule(),"n_spell_s2",d10(1));
                SetLocalInt(GetModule(),"n_spell_s3",d10(1));
                SetLocalInt(GetModule(),"Quest_Strega_Spell",0);
                object oItem = CreateObject(OBJECT_TYPE_PLACEABLE,"col_str_1",GetLocation(GetWaypointByTag("col_str_t1")));
                oItem = CreateObject(OBJECT_TYPE_PLACEABLE,"col_str_2",GetLocation(GetWaypointByTag("col_str_t2")));
                object oRun1 = GetObjectByTag("col_str_1");
                object oRun2 = GetObjectByTag("col_str_2");
                effect eMissile = EffectBeam(VFX_BEAM_LIGHTNING,oRun1,BODY_NODE_CHEST);
                AssignCommand(oRun1,ApplyEffectToObject(DURATION_TYPE_PERMANENT, eMissile, oRun2));
            }
      ...
      if (GetIsPC(oPC2))
        {
                ...
                if(sRef == "set_8_bos"){
                    ...
                    if(nProb == 1){
                        int nProb2 = d2(1);
                        if(nProb2 == 1)
                            CreateItemOnObject("crystal_neg_1",oPC2);
                        else
                            CreateItemOnObject("crystal_pos_1",oPC2);
                    }
                }
                else if(sRef == "set_4_bos"){
                    int nProb = d2(1);
                    if(nProb == 1)
                        CreateItemOnObject("crystal_mag_1",oPC2);
                }
                else if(sRef == "set_25_bos"){
                    int nProb = d2(1);
                    if(nProb == 1){
                        int nProb2 = d2(1);
                        if(nProb2 == 1)
                            CreateItemOnObject("crystal_neg_2",oPC2);
                        else
                            CreateItemOnObject("crystal_pos_2",oPC2);
                    }
                }
                else if(sRef == "set_26_bos"){
                    int nProb = d2(1);
                    if(nProb == 1)
                        CreateItemOnObject("crystal_div_2",oPC2);
                }
                else if(sRef == "set_43_bos"){
                    int nProb = d2(1);
                    if(nProb == 1)
                        CreateItemOnObject("crystal_div_3",oPC2);
                }
                else if(sRef == "set_40_bos"){
                    int nProb = d2(1);
                    if(nProb == 1)
                        CreateItemOnObject("crystal_mag_3",oPC2);
                }
                else if(sRef == "set_41_bos"){
                    int nProb = d2(1);
                    if(nProb == 1)
                        CreateItemOnObject("crystal_neg_3",oPC2);
                }
