...
void DoMissileStorm(int nD6Dice, int nCap, int nSpell,int Diced, int nMIRV = VFX_IMP_MIRV, int nVIS = VFX_IMP_MAGBLUE, int nDAMAGETYPE = DAMAGE_TYPE_MAGICAL, int nONEHIT = FALSE, int nReflexSave = FALSE);

// * Applies ability score damage
void DoDirgeEffect(object oTarget);

void spellsInflictTouchAttack(int nValue, int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID);
...
void DoPetrification(object oSource, object oTarget, int nSpellID, int nFortSaveDC);
...
void DoDirgeEffect(object oTarget)
{
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, GetAreaOfEffectCreator()))
    {
        ...
        if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget))
        {
            ...
            if(!MySavingThrow(SAVING_THROW_FORT, oTarget, nDC, SAVING_THROW_ALL, GetAreaOfEffectCreator()))
            {
                int nGetLastPenalty = GetLocalInt(oTarget, "X0_L_LASTPENALTY");
                // * increase penalty by 2
                nGetLastPenalty = nGetLastPenalty + nBonus;
...
void DoSpikeGrowthEffect(object oTarget)
{
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, GetAreaOfEffectCreator()))
    {
        ...
        {
            int nMetaMagic = GetMetaMagicFeat();
                int nDam = d2(nLevel*3);
                //Enter Metamagic conditions
                if (nMetaMagic == METAMAGIC_MAXIMIZE)
                {
                   nDam = 6*(nLevel);//Damage is at max
                }
           ...
           if (GetHasSpellEffect(SPELL_SPIKE_GROWTH, oTarget) == FALSE)
           {
                ...
                if(!MySavingThrow(SAVING_THROW_REFLEX, oTarget, nDC, SAVING_THROW_ALL, GetAreaOfEffectCreator(), fDelay))
                {
...
void spellsInflictTouchAttack(int nValue,int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID)
{
    ...
    switch(nValue){
        case 0:
            ...
            break;
        case 1:
            nDamage = nNumberRandom(3,nLevel);
            break;
        case 2:
            nDamage = d4(nLevel);
            break;
        case 3:
            nDamage = nNumberRandom(3,nLevel*2);
            break;
        case 4:
            nDamage = d4(nLevel*2);
            break;
                ...
                if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
                {
                    for (i=1; i <= nExtraMissiles + nRemainder; i++)
                    {
                        ...
                        if(Diced==1){
                            ...
                            nDam = nNumberRandom(5,nCasterLvl*2);
                            if (nMetaMagic == METAMAGIC_MAXIMIZE)
                                 nDam = nCasterLvl*10;
                            if (nMetaMagic == METAMAGIC_EMPOWER)
                                  nDam = nDam + nDam/2;
                            if(nSwitch==4)
                                nDam = nDam + 60;
                            float fDamage = (IntToFloat(nDam)/100.0)*50.0;
                            nDam = FloatToInt(fDamage);
                        }
                        else if(Diced==4){
                            nDam = d6(2);
                        ...
                        if(nReflexSave)
                        {
                            ...
                            {
                                nDam = GetReflexAdjustedDamage(nDam, oTarget, nDC, SAVING_THROW_TYPE_ELECTRICITY);
                            }
                            else if ( nDAMAGETYPE == DAMAGE_TYPE_FIRE )
                            {
                                nDam = GetReflexAdjustedDamage(nDam, oTarget, nDC, SAVING_THROW_TYPE_FIRE);
                            }
                        }
...
void DoMagicFang(int nPower, int nDamagePower)
{
    ...
    effect eReduction = EffectDamageReduction(nPower, nDamagePower);
    effect eDur = EffectVisualEffect(VFX_DUR_CESSATE_POSITIVE);
...
void doAura(int nAlign, int nVis1, int nVis2, int nDamageType)
{
    object oTarget = GetSpellTargetObject();
    int nDuration = GetCasterLevel(OBJECT_SELF);
    int nLevel = nDuration/4;
    int nMetaMagic = GetMetaMagicFeat();
    if (nMetaMagic == METAMAGIC_EXTEND)
       nDuration = nDuration * 2;
    effect eVis = EffectVisualEffect(nVis1);
    effect eDur2 = EffectVisualEffect(VFX_DUR_CESSATE_POSITIVE);
    effect eEvil = EffectDamageShield(nLevel, 0, nDamageType);
...
void spellsDispelAoE(object oTargetAoE, object oCaster, int nCasterLevel)
{
    ...
    {
            string sAOETag = GetTag(oTargetAoE);
            ...
            else if(sAOETag == "VFX_PER_WALLBLADE")
                SetLocalInt(oCreator,"AuraDiLame_CURRENT",999999);
            else if(sAOETag == "VFX_PER_STORM")
                SetLocalInt(oCreator,"AuraDiStorm_CURRENT",999999);
            else if(sAOETag == "VFX_PER_GREASE")
                SetLocalInt(oCreator,"AuraDiGrease_CURRENT",999999);
            else if(sAOETag == "VFX_PER_WEB")
                SetLocalInt(oCreator,"AuraDiWeb_CURRENT",999999);
            else if(sAOETag == "VFX_PER_WALLFIRE")
                SetLocalInt(oCreator,"AuraDiWallFire_CURRENT",999999);
            else if(sAOETag == "VFX_PER_STONEHOLD")
                SetLocalInt(oCreator,"AuraDiStone_CURRENT",999999);
            else if(sAOETag == "VFX_PER_EVARDS_BLACK_TENTACLES")
                SetLocalInt(oCreator,"AuraDiEvard_CURRENT",999999);
        vector vAOE = GetPosition(oTargetAoE);
        float f1 = GetLocalFloat(oCreator,"AuraDiVine1_POSITION1");
        float f2 = GetLocalFloat(oCreator,"AuraDiVine1_POSITION2");
        float f3 = GetLocalFloat(oCreator,"AuraDiVine1_POSITION3");
        if(vAOE.x==f1&&vAOE.y==f2&&vAOE.z==f3){
            SetLocalInt(oCreator,"AuraDiVine1_CURRENT",9999999);
        }
        f1 = GetLocalFloat(oCreator,"AuraDiEntangle_POSITION1");
        f2 = GetLocalFloat(oCreator,"AuraDiEntangle_POSITION2");
        f3 = GetLocalFloat(oCreator,"AuraDiEntangle_POSITION3");
        if(vAOE.x==f1&&vAOE.y==f2&&vAOE.z==f3){
            SetLocalInt(oCreator,"AuraDiEntangle_CURRENT",9999999);
        }
        f1 = GetLocalFloat(oCreator,"AuraDiSpike_POSITION1");
        f2 = GetLocalFloat(oCreator,"AuraDiSpike_POSITION2");
        f3 = GetLocalFloat(oCreator,"AuraDiSpike_POSITION3");
        if(vAOE.x==f1&&vAOE.y==f2&&vAOE.z==f3){
            SetLocalInt(oCreator,"AuraDiSpike_CURRENT",9999999);
        }
        f1 = GetLocalFloat(oCreator,"AuraDiVine3_POSITION1");
        f2 = GetLocalFloat(oCreator,"AuraDiVine3_POSITION2");
        f3 = GetLocalFloat(oCreator,"AuraDiVine3_POSITION3");
        if(vAOE.x==f1&&vAOE.y==f2&&vAOE.z==f3){
            SetLocalInt(oCreator,"AuraDiVine3_CURRENT",9999999);
        }
        f1 = GetLocalFloat(oCreator,"AuraDiVine2_POSITION1");
        f2 = GetLocalFloat(oCreator,"AuraDiVine2_POSITION2");
        f3 = GetLocalFloat(oCreator,"AuraDiVine2_POSITION3");
        if(vAOE.x==f1&&vAOE.y==f2&&vAOE.z==f3){
            SetLocalInt(oCreator,"AuraDiVine2_CURRENT",9999999);
        }
