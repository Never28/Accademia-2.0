...
const int LEVEL = 36;
const int DAMAGE_TYPE = DAMAGE_TYPE_BLUDGEONING;
const int DAMAGE_SAVING = SAVING_THROW_TYPE_ALL;
const int VFX_1 = VFX_IMP_ACID_L;
const int VFX_2 = VFX_BEAM_BLACK;
const int VFX_3 = VFX_DUR_AURA_GREEN;
const int VFX_4 = VFX_DUR_AURA_POISON;
...
void TrappolaHeartbeat(object oAOE){
    ...
    while(oTrap!=OBJECT_INVALID||GetLocalInt(oTrap,"not_trap")!=0){
    ...
    while(oTarget!=OBJECT_INVALID){
        if(nDisable == 0){
            if (GetStandardFactionReputation(STANDARD_FACTION_HOSTILE,oTarget) < 11 && GetIsDead(oTarget)==FALSE){
                ...
                TriggerProjectileTrap(SPELL_TRAP_BOLT,oTarget,1,OBJECT_INVALID,oTrap);
