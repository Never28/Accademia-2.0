...
void SafeDestroyObject(object oObject)
{
    ...
    Clean = Clean + 1;
    object oMon = GetFirstObjectInArea(GetArea(oObject));
    while(oMon!=OBJECT_INVALID){
        if(GetIsPC(oMon)==TRUE&&GetIsDM(oMon)==FALSE)
            Clean = 1;
        oMon = GetNextObjectInArea(GetArea(oObject));
    }
    if(Clean==30){
    ...
    ActionDoCommand(SetLocalInt(oObject,"CleanMob",Clean));
    DelayCommand(6.0,SafeDestroyObject(OBJECT_SELF));
}
...
void main()
{
    ...
    if(nAppear != APPEARANCE_TYPE_WEREWOLF && nAppear != APPEARANCE_TYPE_DOG_DIRE_WOLF){
        if(sResRef == "set_45_lml"){
            DelayCommand(3.0,SetName(OBJECT_SELF,"Lupo Mannaro"));
            DelayCommand(3.0,SetCreatureAppearanceType(OBJECT_SELF,APPEARANCE_TYPE_DOG_DIRE_WOLF));
            DelayCommand(3.0,SetPortraitResRef(OBJECT_SELF,"DireWolf_"));
        }
        else if(sResRef == "set_45_lma"){
            DelayCommand(3.0,SetName(OBJECT_SELF,"Lupo Mannaro"));
            DelayCommand(3.0,SetCreatureAppearanceType(OBJECT_SELF,APPEARANCE_TYPE_WEREWOLF));
            DelayCommand(3.0,SetPortraitResRef(OBJECT_SELF,"Werewolf_"));
        }
        else if(sResRef == "set_45_lmf"){
            DelayCommand(3.0,SetName(OBJECT_SELF,"Lupo Mannaro Furioso"));
            DelayCommand(3.0,SetCreatureAppearanceType(OBJECT_SELF,APPEARANCE_TYPE_WEREWOLF));
            DelayCommand(3.0,SetPortraitResRef(OBJECT_SELF,"Werewolf_"));
        }
        else if(sResRef == "set_11_gob"){
            DelayCommand(1.5,AssignCommand(OBJECT_SELF,PlayAnimation(ANIMATION_FIREFORGET_DRINK)));
            DelayCommand(1.0,AssignCommand(OBJECT_SELF,ActionSpeakString("Grazie alla pozione ferrete zchiacciati")));
            DelayCommand(3.0,SetCreatureAppearanceType(OBJECT_SELF,728));
            DelayCommand(3.0,SetCreatureTailType(208,OBJECT_SELF));
        }
        else if(sResRef == "set_12_orc"){
            DelayCommand(1.5,AssignCommand(OBJECT_SELF,PlayAnimation(ANIMATION_FIREFORGET_DRINK)));
            DelayCommand(1.0,AssignCommand(OBJECT_SELF,ActionSpeakString("Grazie alla pozione ferrete zchiacciati")));
            DelayCommand(3.0,SetCreatureAppearanceType(OBJECT_SELF,728));
            DelayCommand(3.0,SetCreatureTailType(212,OBJECT_SELF));
        }
        DelayCommand(3.0,ApplyEffectToObject(DURATION_TYPE_INSTANT,EffectVisualEffect(VFX_IMP_POLYMORPH),OBJECT_SELF));
    }
...
************************* [Other - Behaviour/Generic] *************************/
    ...
    //SetAIInteger(AI_CORPSE_DESTROY_TIME, 60);
...
//    SetSpawnInCondition(NW_FLAG_STEALTH, NW_GENERIC_MASTER);
    if(GetStandardFactionReputation(STANDARD_FACTION_HOSTILE)>=i90 && GetSkillRank(SKILL_HIDE,OBJECT_SELF,TRUE)>GetHitDice(OBJECT_SELF)/i2)
    {
        SetSpawnInCondition(NW_FLAG_STEALTH, NW_GENERIC_MASTER);
