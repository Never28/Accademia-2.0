...
void main()
{
    ...
    //--------------------------------------------------------------------------
    int nNumDice = GetCasterLevel(OBJECT_SELF);
    ...
    while (GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                ...
                nPotential = MaximizeOrEmpower(4, nNumDice*2, nMetaMagic);
                nPotential = nPotential + nBonus;
                float fDamage = (IntToFloat(nPotential)/100.0)*75.0;
                nPotential = FloatToInt(fDamage);
                nDamage    = GetReflexAdjustedDamage(nPotential, oTarget, nDC, nSave);
                //--------------------------------------------------------------
