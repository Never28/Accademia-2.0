...
void main()
{
    ...
    string sMessage = GetName(OBJECT_SELF) + " sta' usando il suo battito d'ali contro di te!";
    location lTarget;
        ...
        if(!GetIsFriend(oTarget) && !GetFactionEqual(oTarget))
        {
            ...
            if(GetCreatureSize(oTarget) != CREATURE_SIZE_HUGE)
            {
                if(!FortitudeSave(oTarget, nDC))
                {
                    ...
                    nDamage = (nDC)*3;
                    eDam = EffectDamage(nDamage, DAMAGE_TYPE_BLUDGEONING,DAMAGE_POWER_PLUS_TWENTY);
