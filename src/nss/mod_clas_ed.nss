...
void SafePolymorph(object oPC, int nRanged, int nMelee, int nShield, int nSpellFailure, string sPolyType, int nPolyType)
{
    ...
    while (GetIsEffectValid(eLoop)){
        if(eEffectType == EFFECT_TYPE_AREA_OF_EFFECT)
    ...
    switch (nEvent){
        ...
        case X2_ITEM_EVENT_ACTIVATE:
            ...
            ||GetAppearanceType(oPC)==APPEARANCE_TYPE_DWARF){
                CastSpell(oPC, GetItemActivatedTarget(), GetItemActivatedTargetLocation(), SPELLABILITY_AURA_FEAR);
            }
            else{
                int eEffectType;
                effect eLoop = GetFirstEffect(oPC);
                ...
                SetLocalInt(oPC,"poly_weaponr",1);
                SetLocalInt(oPC,"poly_weaponm",1);
                SetLocalInt(oPC,"poly_shield",1);
            }
            break;
