void CreaOggetto(object oPC){
    object oItem1;
    object oItem = CreateItemOnObject("set_08_w_arm",oPC);
    oItem1 = CopyItemAndModify(oItem,ITEM_APPR_TYPE_ARMOR_MODEL,ITEM_APPR_ARMOR_MODEL_TORSO,29);
    DestroyObject(oItem);
}
...
void main()
{
    ...
    int nItem1=6,nItem2=4,nItem3=1,nDiff_1,nDiff_2,nDiff_3;
    if(nGold>=2000){
        while(oItem!=OBJECT_INVALID){
            sTag = GetTag(oItem);
            if(sTag=="craft_1_sch"&&nItem1>0){
            ...
            else if(sTag=="craft_8_scw"&&nItem2>0){
