...
void spellsCure(int nValue,int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID)
{
    ...
    switch(nValue){
        case 0:
            nDamage =  nCasterLevel + (nCure/5);
            break;
        case 1:
            nDamage = nNumberRandom(3,nCasterLevel) + (nCure/4);
            break;
        case 2:
            nDamage = d4(nCasterLevel) + (nCure/3);
            break;
        case 3:
            nDamage = nNumberRandom(3,nCasterLevel*2) + (nCure/2);
            break;
        case 4:
            nDamage = d4(nCasterLevel*2) + nCure;
            break;
        case 5:
            nDamage = nNumberRandom(5,nCasterLevel*2) + (nCure*2);
            break;
    }
    int nAlign = GetAlignmentGoodEvil(oTarget);
    //Make metamagic checks
    if (nMetaMagic == METAMAGIC_MAXIMIZE ||
    (nAlign == ALIGNMENT_GOOD && GetItemPossessedBy(OBJECT_SELF,"mod_deity_latha")!=OBJECT_INVALID && GetItemPossessedBy(OBJECT_SELF,"mod_clas_gf")!=OBJECT_INVALID)
    || (nAlign == ALIGNMENT_EVIL && GetItemPossessedBy(OBJECT_SELF,"mod_deity_talos")!=OBJECT_INVALID && GetItemPossessedBy(OBJECT_SELF,"mod_clas_gf")!=OBJECT_INVALID))
    {
        switch(nValue){
            case 0:
                ...
                break;
            case 1:
                nDamage = 3*(nCasterLevel) + (nCure/4);
                break;
            case 2:
                nDamage = 4*(nCasterLevel) + (nCure/3);
                break;
            case 3:
                nDamage = 6*(nCasterLevel) + (nCure/2);
                break;
            case 4:
                nDamage = 8*(nCasterLevel) + (nCure);
                break;
            case 5:
                nDamage = 10*(nCasterLevel) + (nCure*2);
                break;
        }
    }
    ...
    if (GetRacialType(oTarget) != RACIAL_TYPE_UNDEAD && nAspect != APPEARANCE_TYPE_SKELETON_CHIEFTAIN &&
      ...
      && nAspect != APPEARANCE_TYPE_DRACOLICH){
        ...
        effect eVis = EffectVisualEffect(vfx_impactHeal);
...
effect CreateDoomEffectsLink()
{
    ...
    effect eDamage = EffectDamageDecrease(4);
