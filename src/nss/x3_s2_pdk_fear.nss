...
// reports FALSE, do not run this spell
    ...
    int nSave;
    int nCav = GetLevelByClass(CLASS_TYPE_PURPLE_DRAGON_KNIGHT,OBJECT_SELF) + GetLevelByClass(CLASS_TYPE_FIGHTER, OBJECT_SELF);
    nSave = 10 +nCav;
    ...
    float fDuration = RoundsToSeconds(nCasterLevel)*2;// Determine duration based on level
    ...
    while(GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF)&&oTarget!=OBJECT_SELF)
        {
            ...
            {
                //Make a will save
                if(!MySavingThrow(SAVING_THROW_WILL, oTarget, nSave, SAVING_THROW_TYPE_FEAR, OBJECT_SELF, fDelay))
                {
