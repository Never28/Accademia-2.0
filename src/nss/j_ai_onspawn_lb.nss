...
void main()
{
    ...
    if(nAppear != APPEARANCE_TYPE_WEREWOLF && nAppear != APPEARANCE_TYPE_DOG_DIRE_WOLF){
        if(sResRef == "set_11_bos"){
            DelayCommand(1.0,AssignCommand(OBJECT_SELF,PlayAnimation(ANIMATION_FIREFORGET_DRINK)));
            DelayCommand(1.5,AssignCommand(OBJECT_SELF,ActionSpeakString("Grazie alla pozione ferrete zchiacciati")));
            DelayCommand(3.0,SetCreatureAppearanceType(OBJECT_SELF,728));
            DelayCommand(3.0,SetCreatureTailType(206,OBJECT_SELF));
        }
        else if(sResRef == "set_12_bos"){
            ...
            DelayCommand(3.0,SetCreatureAppearanceType(OBJECT_SELF,728));
            DelayCommand(3.0,SetCreatureTailType(214,OBJECT_SELF));
        }
        else if(sResRef == "set_44_bos"){
            DelayCommand(3.0,SetName(OBJECT_SELF,"Sengal il Capo Mannaro (Boss)"));
            DelayCommand(3.0,SetCreatureAppearanceType(OBJECT_SELF,860));
            DelayCommand(3.0,SetPortraitResRef(OBJECT_SELF,"Werewolf_"));
            DelayCommand(3.0,SetCreatureTailType(485,OBJECT_SELF));
        }
