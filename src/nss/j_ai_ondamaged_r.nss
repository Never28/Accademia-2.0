...
void main()
{
    ...
    if(nMax != nCurrent && nAppear != APPEARANCE_TYPE_BODAK){
        SetCreatureAppearanceType(OBJECT_SELF,APPEARANCE_TYPE_BODAK);
        if(sResRef == "set_42_din")
            SetName(OBJECT_SELF,"Doppelganger Infiltrato");
        else if(sResRef == "set_42_dze")
            SetName(OBJECT_SELF,"Doppelganger Zen");
        else if(sResRef == "set_42_dop")
            SetName(OBJECT_SELF,"Doppelganger");
        else if(sResRef == "set_40_bos")
            SetName(OBJECT_SELF,"Ixidor il Millevolti (Boss)");
        SetPortraitResRef(OBJECT_SELF,"Bodak_");
