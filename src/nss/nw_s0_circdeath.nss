...
*/
    ...
    int nMax = nCasterLevel;// maximun hd creature affected, set this to 9 so that a lower HD creature is chosen automatically
    ...
    while ((nHD > 0) && (bContinueLoop))
    {
        ...
        if(bContinueLoop == TRUE)
        {
            ...
            if(!MyResistSpell(OBJECT_SELF, oLowest, fDelay))
            {
                ...
                if(!MySavingThrow(SAVING_THROW_FORT, oLowest, nDC, SAVING_THROW_TYPE_DEATH, OBJECT_SELF, fDelay))
                {
